...
export default function BestTime({ facilityId }: BestTimeProps) {
	...
	if (error)
		return (
			<div className="text-sm text-red-400">Failed to load best times</div>
		);
	if (!data) return null;

	return (
		<div className="rounded-lg border p-4 flex flex-col gap-3">
			<h2 className="text-sm font-medium text-gray-500">Best times to visit</h2>
					...
					<div key={index} className="flex items-center gap-3">
						...
						<div className="flex-1 h-1.5 bg-gray-100 rounded-full overflow-hidden">
							<div
								...
								style={{ width: `${Math.round(slot.avg_occupancy)}%` }}
							/>
						</div>
